...
from pseudocode_summarizer import map_project_folder, remove_gitignored_files, ProjectFile
...
def setup_test_environment(tmp_path: Path) -> Generator[tuple[Path, list[ProjectFile]], any, None]:
    ...
    (tmp_path / ".gitignore").write_text(data="*.txt\n")

    # Create a subdirectory and its .gitignore
    folder = tmp_path / "folder"
    folder.mkdir()
    (folder / ".gitignore").write_text(data="file3.txt\n")

    # Create some files and directories
    d = tmp_path / "dir1"
    d.mkdir()
    p = tmp_path / "file1.txt"
    p.write_text(data="")
    p = d / "file2.txt"
    p.write_text(data="")
    p = folder / "file3.txt"
    ...
    project_files = [
        ...
        ProjectFile(path=Path("dir1/"), modified="2022-01-01 00:00:00"),
...
def test_map_project_folder(setup_test_environment) -> None:
    ...
    expected_paths = sorted(expected_paths, key=lambda x: x.path)
